...
For more information about the components provided with {PlatformNameShort}, see link:http://test/ref-aap-components[{PlatformName} components] in {TitlePlanningGuide}. 

There are several supported installation scenarios for {PlatformName}. To install {PlatformName}, you must edit the inventory file parameters to specify your installation scenario. You can use the link:http://test/rpm-topologies#example_enterprise_inventory_file[enterprise installer] as a basis for your own inventory file.
...
* link:http://test/appendix-inventory-files-vars[Inventory file variables]
